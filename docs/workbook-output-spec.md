...
Use the actual date and shift for each sheet.
...
```text
RN section: 15 staff/blank rows total
...
- Make non-standard start/end values yellow highlighted while keeping standard size 11, non-bold text.
...
```text
Post-fall measures (circle):              Alarm           Nonskid socks             Fall sign             Stay with pt toileting             Fall risk score             Other:____________
```

Keep the prevention-opportunities row on one line because the user only needs to circle items. Separate options with generous spacing rather than pipe characters.
...
- Narrow top, bottom, and right margins.
